...
(function (global, factory) {
  ...
  var isSymbol$2 = USE_SYMBOL_AS_UID$1
    ...
    : function (it) {
        ...
        return (
          isCallable$7($Symbol) &&
          isPrototypeOf($Symbol.prototype, Object$3(it))
        );
      };
  ...
  var objectKeysInternal = function (object, names) {
    ...
    for (key in O)
      !hasOwn$1(hiddenKeys$1, key) && hasOwn$1(O, key) && push(result, key); // Don't enum bug & hidden keys
...
window.addEventListener("DOMContentLoaded", function () {
  ...
  function setCssForAccept() {
    ...
    let styles =
      `
        ...
        form .icon-check{` +
      cssVarCode +
      `position:absolute;top:50%;right:0;width:1em;height:.5em;border-left:2px solid var(--check-mark-color);border-bottom:2px solid var(--check-mark-color);transform:translate(0, -50%) rotate(-45deg);opacity:0}
        form .icon-check.complete{opacity:1}
        form input[type="submit"].disabled{transform:none;pointer-events:none;background-color:#dadada;opacity:0.33}`;
  ...
  function maskInit(enableAccept = true) {
    ...
    if (enableAcceptIcon && !cssInserted) {
      setCssForAccept();
    }
    ...
    document.querySelectorAll('input[type="tel"]').forEach((inputTel) => {
      ...
      submitForm = inputTel.form.querySelector('input[type="submit"]');
      if (submitForm) {
